...
const FavoriteCustomerFiltersComponent = ({ data, actions }: Props) => {
  const { customer } = data;
...
export const FavoriteCustomerFilter = () => {
  const dispatch = useDispatch();
  const { favorites } = useSelector((state) => state.filter);
  const { maxSpentFavorites } = useSelector((state) => state.customer);

  const onChangeName = (name: string) => {
    dispatch(actions.setFavoriteCustomerName(name));
    dispatch(customerActions.favoritesRequest());
  };
  const onChangePhone = (phone: string) => {
    dispatch(actions.setFavoriteCustomerPhone(phone));
    dispatch(customerActions.favoritesRequest());
  };
  const onChangeInvoicing = (min: number, max: number) => {
    dispatch(actions.setFavoriteCustomerInvoicing({ min, max }));
    dispatch(customerActions.favoritesRequest());
  };
  const onChangeStatus = (status: CustomerStatusEnum) => {
    dispatch(actions.setFavoriteCustomerStatus(status));
    dispatch(customerActions.favoritesRequest());
  };

  return (
    <FavoriteCustomerFiltersComponent
      data={{
        customer: {
          name: favorites.name,
          phone: favorites.phone,
          invoicingMin: 0,
          invoicingMax: maxSpentFavorites || 0,
        },
      }}
      actions={{
        onChangeName,
